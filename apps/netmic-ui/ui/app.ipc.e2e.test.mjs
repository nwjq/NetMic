...
test("custom window controls call tauri window APIs", async (t) => {
  ...
  assert.ok(
    tauri.calls.some(
      (call) => call.command === "window_minimize" || call.command === "window.minimize"
    )
  );
  assert.ok(
    tauri.calls.some(
      (call) =>
        call.command === "window_toggle_maximize" ||
...
test("drag handle starts native window dragging", async (t) => {
  ...
  assert.ok(
    tauri.calls.some(
      (call) => call.command === "window_start_drag" || call.command === "window.startDragging"
    )
  );
});
