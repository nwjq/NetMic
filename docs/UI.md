...
## 窗口 / 托盘 / 自启行为
- 主窗口恢复系统原生标题栏与最小化/最大化/关闭按钮，不再保留应用内自定义窗控。
- 系统关闭按钮语义被拦截为：隐藏到后台，不退出应用。
- 页面内容直接从主内容区开始滚动，不再保留额外的应用内标题栏层。
- 为了让真实桌面验收与用户习惯保持一致，`Cmd+W` / `Ctrl+W` 仍然等价于“关闭到后台”。
- 应用提供托盘（macOS 菜单栏 / Linux 系统托盘）入口。
- macOS 托盘图标使用单色 template icon，不复用彩色 app icon，避免菜单栏里糊成一团白块。
- 托盘主开关会根据当前模式切换语义：
  - `Server` 模式：开关“监听”
  - `Client` 模式：开关“推流”
- 托盘同时提供三个显式动作：
