...
## 窗口 / 托盘 / 自启行为
- 主窗口去掉系统原生标题栏与最小化/最大化/关闭按钮，统一使用应用内自定义窗控。
- 自定义窗控不属于页面内容区，而是固定在窗口最上方的标题栏区域；页面滚动时不应跟随内容一起滚动。
- 当前视觉方向对齐“原生标题栏”心智：
  - 窗控固定在左上角，保持贴边；
  - 标题栏除窗控以外的区域都应可拖动；
  - 内容区从标题栏下方开始滚动。
- 应用内窗控语义：
  ...
  - `最大化 / 还原`：切换窗口最大化状态
  - `关闭`：隐藏到后台，不退出应用
- 自定义窗控的点击动作不依赖前端直接访问窗口对象，而是统一通过 Tauri IPC 命令下沉到 Rust 主窗口执行；这样可以规避不同运行形态下前端窗口 API 的兼容差异。
- 窗控按钮的可点击热区需要显著大于视觉圆点本身，避免只剩 14px 小圆点导致真实用户点击困难。
- 为了让真实桌面验收与用户习惯保持一致，应用内窗控同时提供标准快捷键：
  - macOS：`Cmd+M` 最小化，`Ctrl+Cmd+F` 最大化 / 还原，`Cmd+W` 关闭到后台
  - Linux：`Ctrl+M` 最小化，`F11` 最大化 / 还原，`Ctrl+W` 关闭到后台
